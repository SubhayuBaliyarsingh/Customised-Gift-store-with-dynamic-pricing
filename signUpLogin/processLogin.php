...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($errors)) {
        ...
        echo "<script>setTimeout(function() { window.location.href = 'login.php?error=" . urlencode($errorMessage) . "'; }, 2000);</script>";
        exit();
    }
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            echo "<script>setTimeout(function() { window.location.href = 'login.php?error=Invalid email or password'; }, 2000);</script>";
            exit();
        }
    } else {
        // User does not exist
